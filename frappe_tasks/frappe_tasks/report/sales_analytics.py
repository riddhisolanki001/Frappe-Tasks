...
class Analytics(object):
	def __init__(self, filters=None):
		...
		self.sales_person = self.filters.get("sales_person")
	...
	def get_data(self):
		...
		if self.sales_person:
			...
			for rows in self.data:
				entity = rows.get("entity")
				sales_person = frappe.db.get_value(
                "Sales Team",
                {"sales_person": self.sales_person, "parent": entity},
                ["sales_person", "parent"],
                as_dict=1,
            	)
				if sales_person:
		...
		for row in self.data:
			...
			all_sales_person = frappe.db.get_value("Sales Team", {"parent": entity}, "sales_person")
			row["sales_person"] = all_sales_person
